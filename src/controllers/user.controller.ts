...
const verifyOtp: RequestHandler = async (req, res) => {
  try {
    ...
    if (!rollNumber || !otp) {
      res.status(400).json({ success: false, message: "rollNumber and otp are required" });
      return;
    }
    const user = dummyUsers.find((u) => u.rollNumber === rollNumber);
    if (!user || !user.otp || !user.otpExpiresAt) {
      res.status(404).json({ success: false, message: "OTP not found. Please request a new one." });
      return;
    }
